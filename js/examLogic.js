...
// Create modal overlays when the DOM is loaded
document.addEventListener("DOMContentLoaded", () => {
  // Create exam modal overlay
  const examOverlay = document.createElement("div")
  examOverlay.className = "modal-overlay exam-overlay"
  document.body.appendChild(examOverlay)

  // Create explanation modal overlay
  const explanationOverlay = document.createElement("div")
  explanationOverlay.className = "modal-overlay explanation-overlay"
...
export function mostrarModal(elemento) {
  ...
  if (elemento.id === "modal-examen") {
    document.querySelector(".exam-overlay").classList.add("visible")
  } else if (elemento.id === "modal-explicacion") {
    document.querySelector(".explanation-overlay").classList.add("visible")
  }
...
export function ocultarModal(elemento) {
  ...
  if (elemento.id === "modal-examen") {
    document.querySelector(".exam-overlay").classList.remove("visible")
  } else if (elemento.id === "modal-explicacion") {
    document.querySelector(".explanation-overlay").classList.remove("visible")
  }

  // Only restore scrolling if no other modal is visible
  if (!document.querySelector("#modal-examen.visible") && !document.querySelector("#modal-explicacion.visible")) {
...
export class ExamenManager {
  constructor(preguntas, examId, bloque, tema, punto, subpunto, minimoParaAprobar, estructuraGlobal) {
    ...
    if (
      ...
      !this.estructuraGlobal[this.bloque] ||
      !this.estructuraGlobal[this.bloque].puntosLineales
    ) {
  ...
  setupOverlayClicks() {
    // Close modals when clicking on their respective overlays
    const examOverlay = document.querySelector(".exam-overlay")
    if (examOverlay) {
      examOverlay.addEventListener("click", () => {
    ...
    if (explanationOverlay) {
      explanationOverlay.addEventListener("click", () => {
        ocultarModal(document.getElementById("modal-explicacion"))
  ...
  logInitialInfo() {
    ...
    console.log(
      `Bloque: ${this.bloque}, BloqueId: ${this.bloqueId}, Tema: ${this.tema}, Punto: ${this.punto}, Subpunto: ${this.subpunto}`,
    )
  }
  ...
  mostrarExplicacion(esCorrecta, preguntaId) {
    ...
    explicacionContenido.innerHTML =
      this.explicaciones[preguntaId] || "No se encontró una explicación para esta pregunta."

    // Hide exam overlay when showing explanation
    document.querySelector(".exam-overlay").classList.remove("visible")

    // Show explanation modal and its overlay
  ...
  async finalizarExamen() {
    ...
    if (aprobado) {
      ...
      const currentPointIndex = this.estructuraGlobal[this.bloque].puntosLineales.findIndex(
        (punto) => punto.id === currentPointId,
      )
      ...
      if (currentPointIndex !== -1) {
        ...
        console.log(
          `Exam completed. Current point: ${currentPointId}, Next point: ${this.estructuraGlobal[this.bloque].puntosLineales[currentPointIndex + 1]?.id || "No next point"}`,
        )
      } else {
  ...
  async actualizarProgreso() {
    ...
    const puntoActual = this.estructuraGlobal[this.bloque].puntosLineales.find(
      (p) => p.id === this.examId.replace("e", ""),
    )
    if (puntoActual) {
      ...
      if (indicePuntoActual > savedProgress) {
        ...
        console.log(
          `No se actualizó el progreso. Índice actual (${indicePuntoActual}) no es mayor que el guardado (${savedProgress})`,
        )
      }
      ...
      console.log(
        `Exam completed. Current point: ${puntoActual.id}, Next point: ${siguientePunto ? siguientePunto.id : "None"}`,
      )
    }
  }
